...
- 書庫画面の `JSONを書き出す` から、ゴミ箱を除いた現在のノート一式をエクスポートできます。
- 書庫画面の `JSONを取り込む` から、過去に書き出したノート JSON を再取り込みできます。
- 各ノート詳細画面から、そのノートだけを個別 JSON として書き出せます。
